...
def _looks_like_meta_response(text: str) -> bool:
    ...
    return lowered in {"图"} or any(lowered.startswith(prefix) for prefix in meta_prefixes)

def _score_ocr_text(text: str) -> int:
    normalized = _WHITESPACE_RE.sub(" ", text).strip()
    alpha_numeric_count = sum(char.isalnum() for char in normalized)
    token_bonus = min(len(normalized.split()) * 3, 60)
    line_bonus = min(text.count("\n") * 8, 40)
    return alpha_numeric_count + token_bonus + line_bonus
# ---------------------------------------------------------------------------
# Main service class
# ---------------------------------------------------------------------------
...
class OllamaOCRService:
    ...
    OCR_PROMPTS = [
        "Extract all visible text from this medicine label or pharmaceutical document. Return plain text only, preserving line breaks. Do not describe the image.",
        "Read every visible character in the image and return only the recognized text with the original line breaks.",
        "Perform OCR on this label and output only the detected text.",
    ]
    def __init__(
        self,
        ...
        max_retries:     int = 2,
        retry_delay:     float = 0.5,   # base delay; doubles on each retry (exp back-off)
        pdf_workers:     int = DEFAULT_PDF_WORKERS,
        use_tesseract_fallback: bool = True,
    ):
        # Read env vars at instantiation time (after load_dotenv has run)
        ollama_endpoint = ollama_endpoint or os.getenv("OLLAMA_ENDPOINT", "http://localhost:11434")
        model_name      = model_name or os.getenv("OLLAMA_MODEL", "glm-ocr:latest")
        timeout         = timeout if timeout is not None else int(os.getenv("OLLAMA_TIMEOUT", "120"))
        pdf_workers     = pdf_workers if pdf_workers is not None else int(os.getenv("OLLAMA_PDF_WORKERS", "4"))
    ...
    def process_image(self, image_path: str) -> Dict:
        ...
        raw_image_bytes = self._load_raw_image_bytes(image_path)
        text = self._extract_text(
            image_bytes,
            context=image_path,
            raw_fallback_bytes=raw_image_bytes,
        )
        return {
            "extracted_text":  text,
    ...
    def _extract_text(
        ...
        image_bytes: bytes,
        context: str = "",
        raw_fallback_bytes: Optional[bytes] = None,
    ) -> str:
        ...
        try:
            ...
            if raw_fallback_bytes and raw_fallback_bytes != image_bytes:
                logger.warning(
                    "Ollama failed for %s (%s) — retrying with raw RGB image.",
                    context,
                    ollama_err,
                )
                raw_sha = hashlib.sha256(raw_fallback_bytes).hexdigest()
                raw_cached = _cache_get(raw_sha)
                if raw_cached is not None:
                    return raw_cached

                raw_b64 = base64.b64encode(raw_fallback_bytes).decode("utf-8")
                text = self._send_to_ollama(raw_b64, context=f"{context} [raw-rgb]")
                _cache_set(raw_sha, text)
                return text
    ...
    def _send_to_ollama(self, image_base64: str, context: str = "") -> str:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info("🚀 Sending request to Ollama | context=%s | model=%s", context, self.model)
                resp = requests.post(self.api_url, json=payload, timeout=self.timeout)
                logger.info("✅ Got response from Ollama | status=%s", resp.status_code)
    ...
    def _load_raw_image_bytes(self, image_path: str) -> bytes:
        """Load an image and encode it to a simple RGB PNG without thresholding."""
        if self._Image is not None:
            with open(image_path, "rb") as fh:
                raw = fh.read()
    ...
    def _tesseract_fallback(self, image_bytes: bytes, context: str = "") -> str:
        ...
        if not self._tesseract:
            raise RuntimeError("Tesseract fallback is not available in this environment")
